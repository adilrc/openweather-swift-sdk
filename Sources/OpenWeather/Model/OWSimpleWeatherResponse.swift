...
public struct OWSimpleWeatherResponse: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        // Decoding date
        let timeIntervalSince1970: Double = try values.decode(Double.self,
                                                      forKey: .date)
        self.date = Date(timeIntervalSince1970: timeIntervalSince1970)
        
        do {
            self.summaries = try values.decode([OWWeatherConditionSummary].self,
                                               forKey: .summaries)
            self.mainMetrics = try values.decode(OWMainMetrics.self,
                                                 forKey: .mainMetrics)
        } catch {
            // If the basic weather info is not present throw a no content error (error code 204)
            throw OWError.noContent
        }
        self.visibility = try? values.decode(Int.self,
                                             forKey: .visibility)
        self.wind = try? values.decode(OWWind.self,
                                       forKey: .wind)
        self.rain = try? values.decode(OWRain.self,
                                       forKey: .rain)
        self.clouds = try? values.decode(OWClouds.self,
                                         forKey: .clouds)

        self.context = try? values.decode(OWContext.self,
                                          forKey: .context)
